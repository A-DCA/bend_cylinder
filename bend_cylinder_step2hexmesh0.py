...
def identify_geometry(step_file):
    ...
    for dim, tag in surfaces:
        ...
        if abs(length_x) < 1e-6:  # Flat in X (inlet/outlet)
            if abs(xmin) < 1e-6:
                surface_type = "inlet"
            else:
                surface_type = "outlet"
...
def check_periodic_compatibility(inlet_surfaces, outlet_surfaces):
    ...
    for inlet_tag in inlet_surfaces:
        ...
        inlet_center_z = (inlet_bbox[2] + inlet_bbox[5]) / 2
        inlet_center_y = (inlet_bbox[1] + inlet_bbox[4]) / 2
        inlet_x = inlet_bbox[0]  # Should be near 0
        ...
        for outlet_tag in outlet_surfaces:
            ...
            outlet_center_z = (outlet_bbox[2] + outlet_bbox[5]) / 2
            outlet_center_y = (outlet_bbox[1] + outlet_bbox[4]) / 2
            outlet_x = outlet_bbox[0]  # Should be at max Z
            
            # Check if XY centers match (same annular cross-section)
            dz = abs(outlet_center_z - inlet_center_z)
            ...
            if dz < 1e-6 and dy < 1e-6:
                ...
                print(f"      Translation: [{length:.6f}, 0, 0]")
